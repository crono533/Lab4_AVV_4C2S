...
int main(int argc, char* argv[]) {
    ...
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " <1|0> <path> <password>" << std::endl;
        std::cerr << "1 for encryption | 0 for decryption "<< std::endl;
        return 1;
    }

    int32_t mode = std::atoi(argv[1]);
    QString path = argv[2];
    QString password = argv[3];

    QFileInfo info(path);
    auto& manager = CrypterSingleton::instance();

    if (mode == 1) {
        if (info.isDir())
            manager.encrypt_directory(path.toStdString(), password.toStdString());
        else
            manager.encrypt_file(path.toStdString(), password.toStdString());
    } else if (mode == 0) {
        if (info.isDir())
            manager.decrypt_directory(path.toStdString(), password.toStdString());
        else
            manager.decrypt_file(path.toStdString(), password.toStdString());
    } else {
        std::cerr << "Invalid mode: use '1 (encrypt)' or '0 (decrypt)'" << std::endl;
        return 1;
    }
